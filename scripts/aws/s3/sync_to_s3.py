...
USAGE:
    ...
    # Sync default paths (assets, css, phaserjs_editor_scripts_base, src, index.html)
...
def parse_project_path(project_root, project_path, bucket_override=None, prefix_override=None):
    ...
    Paths are synced to: llg-games/games/breaker/{path}/
    - Files at root (e.g., 'index.html') → games/breaker/index.html
    - Directories (e.g., '/src') → games/breaker/src/{files}
    ...
    - s3_prefix: S3 prefix (e.g., 'games/breaker/src/config/themes/' for dirs, 'games/breaker/' for files)
    ...
    if prefix_override:
        ...
        # Build S3 prefix: games/breaker/{project_path}/
...
def sync_to_s3(s3_client, bucket_name, local_dir, local_files, s3_objects, s3_prefix, 
    ...
    if not local_files and len(orphaned_keys) > 0:
        ...
        if folders_deleted == 0:
            ...
            # For directories, sync_scope_prefix ends with '/' (e.g., "games/breaker/test/")
            # For files, sync_scope_prefix is the full S3 key without trailing '/' (e.g., "games/breaker/index2.html")
